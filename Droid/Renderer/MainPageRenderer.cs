...
[assembly: ExportRenderer(typeof(RootTabPage), typeof(MainPageRenderer))]
namespace ReferLocal.Droid
{
	internal class MainPageRenderer : VisualElementRenderer<RootTabPage>, IOnTabClickListener
	{
		...
		protected override void OnElementChanged(ElementChangedEventArgs<RootTabPage> e)
		{
		...
		private void InitializeElement(RootTabPage element)
		{
			PopulateChildren(element);
		}

		private void PopulateChildren(RootTabPage element)
		{
		...
		private void ClearElement(RootTabPage element)
		{
			if (_currentPage != null)
			{
		...
		{
			var barItems = pages.Select(x => new BottomBarTab(Context.Resources.GetDrawable(x.Icon), ""));
